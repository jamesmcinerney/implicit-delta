...
def run(cfg: DictConfig) -> None:
    ...
    from delta.uncertainty import ensemble, idm
    ...
    if cfg.confidence.name == 'ensemble':
        ...
        model = idm.IDMPrediction(create_model, cfg.confidence.n_samples, cfg.confidence.seed_start)
    else:
        raise Exception('confidence method unknown: %s' % cfg.confidence.name)

    model.train(X, Y, stg, cfg, ds_val=val_ds) # train feeding only single example
